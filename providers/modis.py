"""
MODIS Terra (7-2-1) через NASA GIBS WMS -- только для визуального слоя
на карте. Скачивание, отбраковка пустых сцен, ZSTD-сжатие -- как в
исходном ноутбуке.

Облачность больше НЕ считается по пикселям MODIS (был неточный прокси-
метод) -- вместо этого используется облачность из метаданных самих
сцен S2/Landsat (см. providers/copernicus.py, providers/usgs_m2m.py и
readiness.py).
"""
import logging
import os
...
def download_for_aoi(zakaz, aoi_shape, date_str: str):
    """Скачивает MODIS для AOI (буфер 5000 м) и загружает на Google Drive.
    Возвращает ссылку на файл на Drive, либо None, если снимок пуст/недоступен."""
